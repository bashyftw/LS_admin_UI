...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not User.query.filter_by(username='admin').first():
            ...
            dummy_data = [
                {"address": "192.168.1.1", "name": "Controller 1"},
                {"address": "192.168.1.2", "name": "Controller 2"},
                {"address": "192.168.1.3", "name": "Controller 3"},
                # Add more dictionaries for more dummy data
            ]

            # Add the dummy data to the database
            for data in dummy_data:
                controller = Controller(address=data["address"], name=data["name"])
    ...
    multicast_listener(6666, leds_callback)
    multicast_listener(7777, audio_callback)
